...
export default function NotesClient({ initialPage, initialSearch, tag }: NotesClientProps) {
  ...
  return (
    <div className={css.app}>
      ...
      {/* Повідомлення, якщо нотаток за пошуком не знайдено */}
      {data && data.notes.length === 0 && (
        <p className={css.emptyMessage}>
          No notes found for <strong>{initialSearch}</strong>. Create a new one or try another
          search.
        </p>
      )}

      {/* 
        УВАГА: Старий блок модального вікна {isOpenForm && ...} повністю видалено, 
        оскільки логіка створення перенесена на окрему сторінку /notes/action/create.
        Це прибирає синтаксичні помилки парсингу JSX.
      */}
